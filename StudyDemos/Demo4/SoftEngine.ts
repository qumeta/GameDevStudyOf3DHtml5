...
module SoftEngine {
    ...
    export class Device {
        ...
        private drawPoint(point: Qumeta.Vector2, color: Qumeta.Color4) {
        ...
        private processScanLine(y: number, pa: Qumeta.Vector3, pb: Qumeta.Vector3, pc: Qumeta.Vector3, pd: Qumeta.Vector3, color: Qumeta.Color4) {
            var gradient1 = pa.y != pb.y ? (y - pa.y) / (pb.y - pa.y) : 1;
            var gradient2 = pc.y != pd.y ? (y - pc.y) / (pd.y - pc.y) : 1;
            var sx = this.interpolate(pa.x, pb.x, gradient1) >> 0;
            var ex = this.interpolate(pc.x, pd.x, gradient2) >> 0;
            for (var x = sx; x < ex; x++) {
                this.drawPoint(new Qumeta.Vector2(x, y), color);
            }
        }

        private drawTriangle(p1: Qumeta.Vector3, p2: Qumeta.Vector3, p3: Qumeta.Vector3, color: Qumeta.Color4) {
            if (p1.y > p2.y) {
                var temp = p2;
                p2 = p1;
                p1 = temp;
